<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Создать новость - Районные новости</title>
    <link rel="stylesheet" href="/Lesson10/style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
</head>
<body>
<div class="container">
    <nav class="navbar">
        <a href="lesson10.php" class="logo">
            <i class="fas fa-arrow-left"></i> Назад
        </a>
        <div class="nav-links">
            <a href="lesson10.php"><i class="fas fa-home"></i> Главная</a>
        </div>
    </nav>

    <div class="form-container fade-in">
        <h1 style="margin-bottom: 2rem; display: flex; align-items: center; gap: 0.5rem;">
            <i class="fas fa-plus-circle" style="color: #667eea;"></i>
            Создать новую новость
        </h1>

        <form method="POST" action="lesson10.php?action=create">
            <div class="form-group">
                <label for="title">
                    <i class="fas fa-heading"></i> Заголовок новости
                </label>
                <input type="text" id="title" name="title"
                       placeholder="Введите заголовок новости" required>
            </div>

            <div class="form-group">
                <label for="content">
                    <i class="fas fa-align-left"></i> Содержание новости
                </label>
                <textarea id="content" name="content"
                          placeholder="Расскажите, что произошло в нашем районе..."
                          required></textarea>
            </div>

            <div class="form-actions">
                <a href="lesson10.php" class="btn btn-secondary">
                    <i class="fas fa-times"></i> Отмена
                </a>
                <button type="submit" class="btn">
                    <i class="fas fa-paper-plane"></i> Опубликовать новость
                </button>
            </div>
        </form>
    </div>

    <footer class="footer">
        <p>Поделитесь важной информацией с жителями нашего района</p>
    </footer>
</div>
</body>
</html>